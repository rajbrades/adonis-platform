...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {brand.whyChoose.items.map((item) => {
              ...
              return (
                <div key={item.title} className="p-6 rounded-2xl border-2 bg-gray-800/80 border-gray-700 text-center">
                  <div 
                    className="mb-4 w-16 h-16 mx-auto rounded-xl flex items-center justify-center"
                    style={{ backgroundColor: `${brand.colors.primary}10` }}
                  >
                    <Icon className="w-8 h-8" style={{ color: brand.colors.primary }} />
                  </div>
      ...
      {/* Why ADONIS/ATHENA - 3 items */}
      <section className="py-24 px-6 bg-black/20">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-5xl md:text-6xl font-bold text-center mb-16 text-white">
            Why <span style={{ color: brand.colors.primary }}>{brand.name}</span>
          </h2>

          <div className="grid md:grid-cols-3 gap-8">
            {brand.whyUs.items.map((item) => {
              const Icon = iconMap[item.icon as keyof typeof iconMap]
              return (
                <div key={item.title} className="p-6 rounded-2xl border-2 bg-gray-800/80 border-gray-700">
                  <Icon className="w-12 h-12 mb-4" style={{ color: brand.colors.primary }} />
                  <h3 className="text-xl font-bold mb-3 text-white">{item.title}</h3>
      ...
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="rounded-3xl border-2 border-gray-800 bg-gray-900/50 p-16">
            ...
            <p className="text-xl text-center mb-16 max-w-3xl mx-auto text-gray-400">
            ...
            <div className="grid md:grid-cols-4 gap-12">
              {brand.stats.items.map((stat) => (
                <div key={stat.label} className="text-center">
                  <div className="text-5xl md:text-6xl font-bold mb-3" style={{ color: brand.colors.primary }}>
                    {stat.value}
                  </div>
